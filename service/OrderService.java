...
public class OrderService {
	...
	public List<Orders> getList(byte status, int page, int row) {
		...
		if (status == 0) {
			orderList = orderDao.getList(row * (page-1), row);
		}else{
			orderList = orderDao.getListByStatus(status, row * (page-1), row);
		}
		if ( orderList != null ){
			for(Orders order : orderList) {
				order.setItemList(this.getItemList(order.getId()));
				order.setUser(this.userService.get(order.getUserId()));
			}
		}
		return orderList;
	}
	 ...
	 * @return
	 */
	public List<Orders> getDayList(String day) {
		List<Orders> orderList = null;
		orderList = orderDao.getDayList(day);
		if ( orderList != null ){
			for(Orders order : orderList) {
				order.setItemList(this.getItemList(order.getId()));
				order.setUser(this.userService.get(order.getUserId()));
